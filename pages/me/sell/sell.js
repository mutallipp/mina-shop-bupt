...
Page({
  data: {
    order_list: ['', ''],
    statusType: ["在架宝贝", "待发货", "交易中", "已完成","下架宝贝"],
    currentType: 0,
    tmp_type:1
  },
  ...
  get_data: function () {
    var t = this;
    utils.net('/member/buy-list', { 'act': 'sell'},
      function (e) {
        t.setData({
  ...
  actOrder: function (e) {
    ...
    if (act =='xiajia'){
      ...
      app.console('edit')
      wx.navigateTo({
        url: '/pages/guidepublish/upload/upload?q=' + '1' + '&good_id=' + good_id,
      })
      return;
    } if (act == 'cSell') {
      app.console('cSell')
      ...
      utils.net('/get-good-info', { 'good_id': good_id },
            function (e) {
              if(e.code==200){
                wx.makePhoneCall({
                  phoneNumber: e.data.good_info.phone // 仅为示例，并非真实的电话号码
                })
              }else{
                app.console(e.msg)
              }
